...
def get_last_n_commits_diffs(n: int = 2):
  ...
  try:
    ...
    commit_hashes_output = subprocess.check_output(["git", "log", f"-{n}", "--pretty=format:%H"],
    ...
    for commit_hash in commit_hashes_output:
      ...
      diff_output = subprocess.check_output(["git", "show", commit_hash], stderr=subprocess.STDOUT).decode('utf-8')

      # Get commit message for this commit
      commit_msg = subprocess.check_output(["git", "log", "-1", "--pretty=format:%s", commit_hash],
                                           stderr=subprocess.STDOUT).decode('utf-8')

      diffs.append({"hash": commit_hash, "message": commit_msg, "diff": diff_output})
