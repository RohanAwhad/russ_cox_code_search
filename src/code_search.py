...
def _mandatory_trigrams(subpat, required=True) -> set[str]:
  ...
  while i < len(data):
    ...
    # alternation  (X|Y)  ->  intersection of every branch's guarantees
      ...
      branches = val[1]
      branch_sets = [_mandatory_trigrams(_subpattern_from_data(subpat.state, b)) for b in branches]
      if branch_sets:
...
class TrigramRegexSearcher:
  ...
  def search(self, pattern: str) -> list[int]:
    ...
    return [doc for doc in candidates if rx.search(self.docs[doc])]  # type: ignore
